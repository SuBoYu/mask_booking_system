								...
								<select class="form-control" name="shop" id="shop">
									...
									<?php
										...
										foreach($workAt as $row) {
											if(isset($_POST['shop']) and !strcmp($shop_condition, $row[0]))
					...
					<div>
						<?php
							if(isset($error['delete_order_error'])) {
								echo "<script>alert(\"${error['delete_order_error']}\")</script>";	
								echo '<p class="bg-danger">'.$error['delete_order_error'].'</p>';
							} else if(isset($error['finish_order_error'])) {
								echo "<script>alert(\"${error['finish_order_error']}\")</script>";	
								echo '<p class="bg-danger">'.$error['finish_order_error'].'</p>';
							}
							$sql = "SELECT OID, status, create_time, end_time, name, o.price, o.amount FROM mask_order as o join store as s WHERE o.SID=s.SID and o.SID in (SELECT distinct store.SID FROM staff join store WHERE staff.UID=:UID and staff.SID=store.SID or store.UID=:sUID)";
							$in_params[':UID'] = $_SESSION['UID'];
							$in_params[':sUID'] = $_SESSION['UID'];
							if(strcmp($shop_condition, "all")) {
								$sql .= " and s.SID=:shop_condition";
								$in_params[':shop_condition'] = Database::get()->execute("SELECT SID FROM store WHERE name = :name", array(":name"=>$shop_condition))[0][0];
							}
							...
							foreach($shopOrder as $order){
								...
								if($order['status'] == "unfinished") {
									echo "<input type='hidden' name='FOID' value='" . $order['OID'] . "'>" . "<input type='submit' class='btn btn-primary btn-lg'  name='finish_order' value='finish' style='background-color:green;'>";
									echo "<input type='hidden' name='COID' value='" . $order['OID'] . "'>" . "<input type='submit' class='btn btn-primary btn-lg' name='cancel_order' value='cancel' style='background-color:red;'>";
								}
								echo "</td></tr>";
								echo "</form>";
							}
